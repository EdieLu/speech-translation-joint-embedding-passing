...
class Trainer_AE_MT(Trainer):
	...
	def _evaluate_batches(self, model, dataset):
		...
		with torch.no_grad():
			for idx in range(len(evaliter)):
				...
				for bidx in range(n_minibatch):
					...
					# preds_en = out_dict['preds_ae']
					# logps_en = out_dict['logps_ae']
					# logps_hyp_de = logps_de[:,:-1,:]
					# preds_hyp_de = preds_de[:,:-1]
					# logps_hyp_en = logps_en[:,1:,:]
					# preds_hyp_en = preds_en[:,1:]
					...
					out_count = self._print_hyp(out_count, src_ids, tgt_ids,
						dataset.src_id2word, dataset.tgt_id2word, seqres_en, seqres_de)
		...
		resloss_en *= self.loss_coeff['nll_ae']
	...
	def _train_batch(self,
		...
		for bidx in range(n_minibatch):
			...
			out_dict = model.forward_train(src_ids, tgt=tgt_ids,
				mode='AE_MT', use_gpu=self.use_gpu)
